...
class CrxMake < Object
  ...
  def check_valid_option o
    @exdir, @exbufs, @pkey, @pkey_o, @crx, @verbose, @ignorefile, @ignoredir = o[:ex_dir], o[:ex_buffers], o[:pkey], o[:pkey_output], o[:crx_output], o[:verbose], o[:ignorefile], o[:ignoredir]
...
ext dir: \"#{@exdir}\"
  ...
  def check_valid_option_zip o
    @exdir, @exbufs, @pkey, @pkey_o, @zip, @verbose, @ignorefile, @ignoredir = o[:ex_dir], o[:ex_buffers], o[:pkey], o[:pkey_output], o[:zip_output], o[:verbose], o[:ignorefile], o[:ignoredir]
    @exdir = File.expand_path(@exdir) if @exdir
    raise "extension dir not exist" if !@exdir || !File.exist?(@exdir) || !File.directory?(@exdir)
    @pkey = File.expand_path(@pkey) if @pkey
    raise "private key not exist" if @pkey && (!File.exist?(@pkey) || !File.file?(@pkey))
...
ext dir: \"#{@exdir}\"
  ...
  def create_zip
    ...
    Zip::Archive.open(@zip, Zip::CREATE | Zip::TRUNC) do |zip|
      ...
      @exbufs.each do |buf, name|
        zip.add_buffer(name, buf)
      end
